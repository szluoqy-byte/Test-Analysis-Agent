...
历史报告如果没有触发过交互澄清，可缺省 `交互澄清摘要`；新报告必须保留该章节并写明“未触发交互澄清”和未触发原因。
...
- `类型` 必须来自 `knowledge/testpoint-standard.md` 的标准类型。
- `方法` 必须来自 `knowledge/testpoint-standard.md` 的标准方法或明确标记为启发式方法。
- `级别` 必须来自 `knowledge/testpoint-standard.md` 的级别取值。
