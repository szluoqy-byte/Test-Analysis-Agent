...
Memory 是 Agent 在多次需求分析之间保留的、经人工确认的项目上下文、测试经验和个人本地偏好。它的目标是让测试用例设计输入更贴近当前项目和当前使用者，而不是替代需求文档、测试理论或专家知识库。
...
| personal | `user/**/*.md` | 否 | 当前使用者的个人输出偏好、检查习惯和本地补充；路径继续兼容历史 `user` 目录 |
...
7. 后续需求分析、方法路由、设计输入生成和覆盖审查默认读取当前 run 的 `context-pack.md`；如果上下文不足，可以按 context pack 记录的来源文件或当前需求明确指向的 project/personal 文件受控补读相关章节。
