...
Memory 是 Agent 在多次需求分析之间保留的、经人工确认的项目上下文和测试经验。它的目标是让测试用例设计输入更贴近当前项目，而不是替代需求文档、测试理论或专家知识库。
...
| user | `user/**/*.md` | 否 | 当前使用者的个人输出偏好、检查习惯和本地补充 |
...
| `user/**/*.md` | 当前使用者的个人偏好、检查清单和本地记忆 | 自动扫描，按需注入；默认不提交 Git |
...
4. 扫描 `user/**/*.md`，只选择与当前需求直接相关的个人偏好或本地检查关注点。
...
7. 后续需求分析、方法路由、设计输入生成和覆盖审查默认读取当前 run 的 `context-pack.md`；如果上下文不足，可以按 context pack 记录的来源文件或当前需求明确指向的 project/user 文件受控补读相关章节。
8. 过程分析报告给出“建议沉淀的 Memory 更新”。
9. 用户确认后，才把建议追加到对应长期 memory 文件、业务域分片、项目化 memory 文件或 user 本地文件。
...
- user memory 不需要登记索引；新增 `user/**/*.md` 后会按需扫描，但只能作为个人偏好或本地检查补充。
- 无法唯一确定 `project-key` 时，不得读取所有项目目录正文；项目归属问题应进入最终待确认信息。
- user 层不保存项目事实、团队共识和真实缺陷复盘；这些内容应进入 project memory 或待确认问题。
