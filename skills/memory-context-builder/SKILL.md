...
- 可选 `personal-key`：来自用户显式参数、当前运行者配置或默认个人配置；当前兼容目录为 `*/user/`，后续可扩展为 `*/user/<personal-key>/`。
...
配置按 `core / project / personal` 三层处理，当前路径继续兼容历史 `user` 目录：
...
3. 未提供时使用默认 personal 配置，扫描历史兼容路径 `*/user/**/*.md`。
