...
- 运行阶段顺序来自 `templates/task-list-template.md`，当前 run 的 `process/task-list.md` 是流程事实源。
...
本流程按 `core / project / personal` 三层读取配置。core 层是随 Agent 包发布的根目录文件；project 层是 `*/projects/<project-key>/**/*.md`；personal 层当前兼容路径是 `*/user/**/*.md`。project 和 personal 层默认不提交 Git，只作为本地 overlay。
...
personal 文件发现范围当前使用历史兼容路径 `*/user/**/*.md`。如果用户或运行环境提供了 `personal-key`，必须在 `context-pack.md` 记录；如果未来存在 `*/user/<personal-key>/` 子目录，优先使用该子目录，否则使用默认 personal。

如果无法唯一确定 `project-key`，不得全量读取所有项目目录正文；继续使用 core 层和 personal 层，并把项目归属问题登记到最终“待确认信息”。project/personal 层只能补充项目风险画像、覆盖策略、术语映射、路由说明、测试 oracle、模板说明、个人偏好或附加门禁，不得覆盖 core 层中的核心标准、字段、类型、级别、输出契约和质量门禁。personal 层也不得覆盖需求文档或 project memory，不得作为项目事实或团队共识。
...
当专项 skill 发现 context pack 缺少完成当前分析所需的 project/personal 信息时，可以按源补读，但必须受控：

- 优先读取 context pack 已列出的来源文件、章节、关键词或“后续补读建议”。
- 如果当前需求明确指向某个 project/personal 文件，也可以读取该对应文件。
- 不得自行全目录搜索 project/personal，也不得把大文件整份复制进过程产物。
- 读取结果直接进入当前 skill 的方法证据、风险备注或过程报告，不要求刷新 `context-pack.md`。

超过 50KB 的 project/personal Markdown 不要求提供 `index.md`。读取大文件时先看文件名、frontmatter、标题结构或目录，再按命中的标题、关键词或表格片段读取必要内容；如果仍无法定位，转为待确认候选或风险备注。
...
3. 立即使用 `templates/task-list-template.md` 创建 `${PROJECT_ROOT}/outputs/runs/<run-id>/process/task-list.md`，并将阶段 1 标记为 `done`。后续每个阶段开始前更新为 `in_progress`，完成后更新为 `done`、`skipped` 或 `blocked`。
4. 解析可选 `project-key` 和 `personal-key`，使用 `memory-context-builder` 扫描 core、project 和 personal 三层配置，生成本次运行的 `${PROJECT_ROOT}/outputs/runs/<run-id>/process/context-pack.md`，并登记可能的 memory、项目归属或 personal 来源待确认候选。context pack 必须展示 project/personal 的绑定结果、命中来源、未采用来源和后续补读建议。
5. 在 `CP-MEMORY` 检查点使用 `clarification-gate`；memory 冲突影响需求理解时登记为待确认候选，不中途提问。
6. 使用 `requirement-testability` 生成结构化需求模型，并按 `knowledge/test-analysis-methodology.md` 标记需求片段涉及的测试分析维度，登记业务规则、状态、权限、边界和接口契约待确认候选。
7. 在 `CP-REQUIREMENT` 检查点必须使用 `clarification-gate`；这是主要待确认收集点，必须形成候选队列、去重排序结果和最终保留原因。即使存在 `P0/P1` 问题，也不得中途打断用户。
8. 使用 `testing-method-router` 先判断每个需求片段涉及的分析维度，再选择适用测试方法，并登记影响方法必要性的待确认候选。
9. 在 `CP-ROUTING` 检查点使用 `clarification-gate`；性能、安全、兼容等高优先范围问题如果缺少依据，进入最终待确认候选。
10. 按方法路由调用必要的专项 skill：
...
11. 汇总专项 skill 产出的 `ME-*` 方法分析证据，并登记专项方法缺口待确认候选。
12. 如果任一阶段发现 context pack 不足，可按上述受控规则补读对应来源文件；补读仍不足时转为待确认候选或风险备注；如未触发补读，将任务清单中“按源补读”阶段保持 `skipped` 并说明原因。
13. 在 `CP-METHOD` 检查点使用 `clarification-gate`；会导致决策表、状态图、权限矩阵或接口契约失真的缺口必须进入最终待确认候选。
14. 使用 `testpoint-generation` 基于方法证据生成场景化测试用例设计输入：先识别测试场景，再补齐场景测试条件、场景测试点、接口测试清单和接口测试点；原则上不在本阶段打断用户，缺口写为待确认风险点。
15. 使用 `coverage-review` 执行覆盖审查、质量门禁和专家评分，并登记阻断报告发布的待确认候选。
16. 在 `CP-REVIEW` 检查点使用 `clarification-gate`；对仍会影响交付可用性的缺口做最终收口，不提问、不暂停。
17. 根据待确认候选、结构化需求模型、方法证据和覆盖审查结果，刷新最终“待确认问题”：移除已覆盖和重复的问题，只保留后续用例设计必须知道的未解决问题。
18. 如果质量门禁因输出质量失败，修正后重新审查；如果因需求信息缺失失败，保留刷新后的失败项并生成“待确认问题”。
19. 将主输出写入 `${PROJECT_ROOT}/outputs/runs/<run-id>/deliverables/testcase-design-input.md`，使用 `templates/testcase-design-input-template.md`，供后续独立测试设计项目直接消费。
20. 如需保留过程审查信息，将分析报告写入 `${PROJECT_ROOT}/outputs/runs/<run-id>/reports/test-analysis-report.md`，报告中可包含 project/personal 上下文使用情况、方法路由、方法证据、覆盖审查、质量门禁、专家评分和 memory 更新建议。
21. 最终输出前刷新 `process/task-list.md`：所有必选阶段必须为 `done`，未触发的可选阶段为 `skipped` 并说明原因；如果存在 `blocked`，必须在最终待确认信息或过程报告中说明。
...
| `memory-context-builder` | `process/context-pack.md`、project/personal 来源使用摘要、memory 待确认候选 | 需求可测性、待确认治理 |
...
- 如保留过程分析报告，使用 `templates/final-report-template.md`，并在报告中记录 `PROJECT_ROOT`、`run-id`、运行目录、设计输入路径、分析报告路径、待确认治理记录路径、上下文包路径，以及 project/personal 上下文使用情况。
