...
- 结构化需求模型。
- 记忆上下文包。
- 记忆上下文包中命中的 project/user 覆盖策略、风险画像和路由补充。
...
2. 识别每个分析维度下的触发信号。
3. 结合 context pack 中的 project/user knowledge 补充识别项目特有风险、个人关注点或覆盖要求；补充只能提高关注度，不能覆盖根目录 knowledge 的路由矩阵和核心标准。
4. 将触发信号映射到一个或多个测试方法。
...
- project/user knowledge 补充以当前 run 的 `process/context-pack.md` 为准，只能补充关注点和原因；user 层不能覆盖 project 层或 core 层约束。
...
- 如果 context pack 中的 project/user 覆盖策略不足，只能按 context pack 记录的来源或当前需求明确指向的文件补读相关章节，并在路由说明中记录来源；不得全目录搜索或全量复制大文件。
